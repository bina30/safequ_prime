...
{
    public static function sendSMS($to, $from, $text, $template_id, $variables = [])
    {
        if (OtpConfiguration::where('type', 'msg91')->first()->value == 1) {

            // send

            $response = Msg91::sms()
                ->to($to) // set the mobile with country code
                ->flow($template_id); // set the flow id
            if (!empty($variables)) {
                foreach ($variables as $key => $value) {
                    $response->variable($key, $value);
                }
            }
